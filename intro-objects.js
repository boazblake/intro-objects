...
function getFullNames(customers) {
    ...
    for (var i = 0; i < customers.length; i++) {
        fullNamesArray = customers[i].first + ' ' + customers[i].last
        console.log(fullNamesArray)
        inputObjectsArray.push(fullNamesArray);
...
function getCounts(stringOfWords) {
    ...
    for (var i = 0; i < arrayOfWords.length; i++) {
        ...
        if (wordsObject.hasOwnProperty(word) === true) {
            wordsObject[word] = wordsObject[word] + 1 // same as wordsObject += 1
        } else {
            wordsObject[word] = 1
        }
